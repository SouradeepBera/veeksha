...
class OpenAIChatCompletionsClient(OpenAIBaseClient):
    ...
    async def _process_stream(self, response: aiohttp.ClientResponse):
        ...
        # Framed on bytes: ``iter_any`` hands over whatever has arrived, so
        # decoding per network chunk could split a multi-byte character across
        # the boundary. ``json.loads`` reads bytes directly.
        buffer = b""
